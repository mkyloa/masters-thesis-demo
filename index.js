...
<span class="code-comment">// Очікуваний результат: BUILD SUCCESS (Вразливість недосяжна)</span>, result: 'success' }, vuln: { id: 'vuln', code: <span class="code-keyword">import</span> { <span class="code-function">dangerousEval</span> } <span class="code-keyword">from</span> <span class="code-string">'legacy-lib'</span>;
...
<span class="code-comment">// Очікуваний результат: BUILD FAILED (Знайдено шлях експлуатації)</span>, result: 'fail' }, unused: { id: 'unused', code: <span class="code-keyword">import</span> * <span class="code-keyword">as</span> <span class="code-variable">lib</span> <span class="code-keyword">from</span> <span class="code-string">'legacy-lib'</span>;
...
function setScenario(id) {
    ...
    document.getElementById('code-display').innerHTML = scenarios[id].code;
    document.getElementById('terminal-output').innerHTML = `<div class="text-slate-600">> Ready to build...</div>`;
    document.getElementById('term-status').innerText = 'Idle';
    document.getElementById('term-status').className = 'text-slate-500';
}
...
async function runBuild() {
    ...
    document.getElementById('term-status').className = 'text-yellow-400 animate-pulse';
    ...
    if (currentScenario === 'safe') {
        ...
        document.getElementById('term-status').innerText = 'Success';
        document.getElementById('term-status').className = 'text-green-500 font-bold';
        ...
        document.getElementById('term-status').innerText = 'Failed';
        document.getElementById('term-status').className = 'text-red-500 font-bold';
    } else {
...
function appendLog(container, text, classes, timestamp) {
    return new Promise(resolve => {
        setTimeout(() => {
            ...
            resolve();
        }, timestamp - (timestamp > 500 ? 0 : 0)); // Simplified logic, acts as sequence delay
    });
}